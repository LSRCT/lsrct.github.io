...
The goal here is not to replace humans, at least not yet, but to go one step above the current assistants. Currently, work with AI is very interactive, you ask a question and get an answer within a few seconds. In a way that's like having a conversation with a person, when what we really want is to delegate a task and send the LLMs to work. Imagine asking an AI to plan the company Christmas party and it not only looks for a good date, but it also orders food and snacks in line with the theme it gave the party and sends out invitations, all without any oversight needed.
